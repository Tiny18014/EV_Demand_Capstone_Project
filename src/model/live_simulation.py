...
MODELS_DIR = Path(__file__).parent.resolve()  # This is src/model/
HYBRID_MODEL_PATH = MODELS_DIR / "advanced_model_monthly_hybrid.pkl"

warnings.filterwarnings('ignore')
# --- IMPORT LOGIC (The Fix) ---
# We append the model directory to sys.path so Python can find adjacent files
...
def precompute_all_predictions():
    ...
    if not HYBRID_MODEL_PATH.exists():
        print(f"❌ Model file not found at: {HYBRID_MODEL_PATH}")
        print("   Please run advanced_model_trainer.py first to generate it.")
        return pd.DataFrame()

    with open(HYBRID_MODEL_PATH, 'rb') as f:
        models_data = pickle.load(f)
        
    # 3. Aggregate & Create Features
    ...
    for cat, model_data in models_data.items():
        ...
        # Filter for category and valid states
        cat_df = df_features[
            (df_features['Vehicle_Category'] == cat) & 
            (df_features['State'].isin(train_states))
        ].copy()
        ...
        # Predict Monthly
        cat_df['Predicted_Monthly'] = np.maximum(0, model.predict(X_scaled))
    ...
    # Merge with original data to get Actuals
    merged_df = pd.merge(
...
def run_simulation(stop_event=None):
    ...
    if augmented_df.empty:
        print("❌ Error: No predictions generated. Stopping simulation.")
        return
    ...
    try:
        ...
        while True:
            ...
            if stop_event and stop_event.is_set():
                print("🛑 Simulation stopping command received.")
                break
            ...
            try:
                ...
                print(f"⚠️ Insert Error: {inner_e}")

            # WAIT: Non-blocking sleep checking for stop signal
            if stop_event and stop_event.wait(timeout=1.5):
                break
            elif not stop_event:
                time.sleep(1.5)
